...
// Elligator 2
    ...
    FP_BLS12381_one(&one);
    FP_BLS12381_add(&t,&t,&one); FP_BLS12381_norm(&t);
    FP_BLS12381_inv(&t,&t,NULL);      // 1/(1+z.t^2)
    FP_BLS12381_mul(&X1,&t,&A);
    FP_BLS12381_neg(&X1,&X1);
    FP_BLS12381_copy(&X2,&X1);
    FP_BLS12381_add(&X2,&X2,&A); FP_BLS12381_norm(&X2);
    FP_BLS12381_neg(&X2,&X2);

    ECP_BLS12381_rhs(&t,&X2);

    FP_BLS12381_cmove(&X1,&X2,FP_BLS12381_qr(&t,NULL));
...
// Elligator 2 - map to Montgomery, place point, map back
    int qres,sgn,ne,rfc;
    BIG_384_58 x,y;
    FP_BLS12381 X1,X2,t,one,A,w1,w2,B,Y,K,NY;
    FP_BLS12381_one(&one);
...
// Map to this Montgomery curve X^2=X^3+AX^2+BX
    ...
    FP_BLS12381_copy(&t,h); sgn=FP_BLS12381_sign(&t);
    FP_BLS12381_sqr(&t,&t);   // t^2
    ...
    FP_BLS12381_norm(&X1);
    FP_BLS12381_sqr(&t,&X1); FP_BLS12381_mul(&w1,&t,&X1);  // t=X1^2, w1=X1^3
    FP_BLS12381_mul(&t,&t,&A); FP_BLS12381_add(&w1,&w1,&t);
    if (!rfc)
    {
        FP_BLS12381_mul(&t,&X1,&B);
        FP_BLS12381_add(&w1,&w1,&t);   // w1=X1^3+AX1^2+BX1
    } else {
        FP_BLS12381_add(&w1,&w1,&X1);
    }
    FP_BLS12381_norm(&w1);

    FP_BLS12381_norm(&X2);
    FP_BLS12381_sqr(&t,&X2); FP_BLS12381_mul(&w2,&t,&X2);  // t=X1^2, w2=X1^3
    FP_BLS12381_mul(&t,&t,&A); FP_BLS12381_add(&w2,&w2,&t);
    if (!rfc)
    {
        FP_BLS12381_mul(&t,&X2,&B);
        FP_BLS12381_add(&w2,&w2,&t);   // w2=X1^3+AX1^2+BX1
    } else {
        FP_BLS12381_add(&w2,&w2,&X2);
    }
    FP_BLS12381_norm(&w2);

    qres=FP_BLS12381_qr(&w2,NULL);
    FP_BLS12381_cmove(&X1,&X2,qres);
    FP_BLS12381_cmove(&w1,&w2,qres);

    FP_BLS12381_sqrt(&Y,&w1,NULL);

    FP_BLS12381_neg(&NY,&Y); FP_BLS12381_norm(&NY);
    FP_BLS12381_cmove(&Y,&NY,1-qres);
...
// GOLDILOCKS isogeny
    ...
    FP_BLS12381_add(&NY,&t,&one); // NY=u^2+1
    FP_BLS12381_norm(&NY);
    ...
    FP_BLS12381_mul(&X1,&X1,&t);

    if (rfc)
        FP_BLS12381_mul(&X1,&X1,&K);

    FP_BLS12381_mul(&Y,&Y,&w2);
    FP_BLS12381_mul(&Y,&Y,&t);
#endif
    FP_BLS12381_redc(x,&X1);
...
// SWU method.
    ...
    sgn=FP_BLS12381_sign(&t);
        ...
        FP_BLS12381_add(&w,&t,&one);
        FP_BLS12381_norm(&w);

        FP_BLS12381_mul(&w,&w,&t);    // w=t2(t2+1)
        FP_BLS12381_mul(&NY,&A,&w);     // A=Aw
        FP_BLS12381_inv(&NY,&NY,NULL);
...
// Shallue and van de Woestijne
        ...
        FP_BLS12381_mul(&NY,&t,&Y);
        FP_BLS12381_mul(&NY,&NY,&B);

        FP_BLS12381_copy(&w,&A);
        FP_BLS12381_tpo(&NY,&w);   // tv3=inv0(tv1*tv2*z*sqrt(-3)) and sqrt(g(Z)) 
        ...
        FP_BLS12381_mul(&w,&w,h);
        FP_BLS12381_mul(&w,&w,&Y);
        FP_BLS12381_mul(&w,&w,&NY);     // tv5=u*tv1*tv3*tv4*Z*sqrt(-3)
