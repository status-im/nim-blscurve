...
proc mulCoFactorIter*(point: ECP2_BLS381): ECP2_BLS381 =
  ## Multiplies point by a cofactor of G2
  let p = point
  result = point
...
proc mulCoFactorRec*(point: ECP2_BLS381, n: string = G2_CoFactorBit): ECP2_BLS381 =
  ## Multiplies point by a cofactor of G2
  ## Not a good approach, of course. Uses 500+ stack depth
  if n == "0":
    result.inf()
  elif n == "1":
    result = point
    ...
    result = point
    double(result)
    result = mulCoFactorRec(result, n[0 .. ^2])
  elif n[^1] == '1':
    result = point
    double(result)
    result = mulCoFactorRec(result, n[0 .. ^2])
