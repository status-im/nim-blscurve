...
suite "[SCHEME3] BLS381 test suite (public interface)":
  ...
  test "Sign/Serialize/Deserialize/Verify test":
    ...
    var g1 = G2_CoFactorHigh
    var g2 = G2_CoFactorLow
    var g3 = G2_CoFactorShift
    norm(g1)
    norm(g2)
    norm(g3)
    echo "G2_CoFactorHigh = ", $g1
    echo "G2_CoFactorLow = ", $g2
    echo "G2_CoFactorShift = ", $g3

    var sigkey = initSigKey("0000000000000000000000000000000015DAAB8E0DAD718751768A2DCC33F601F769A0CE26C954E45561403CC8CF66BF")
    var verkey = sigkey.fromSigKey()

    var message = "Simple message"
    # for i in 0..<100:
    var desig: Signature
    var idomain = uint64(5)
    var sig = sigkey.signMessage(idomain, message)
    var serialized = sig.getRaw()
    echo "ITERATION ", 5
    echo dumpHex(serialized)
    echo repr sig
    check Signature.fromRaw(serialized, desig) == true
    echo repr desig
    check desig.verifyMessage(message, idomain, verkey) == true
